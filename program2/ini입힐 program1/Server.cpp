...
{
	//Cini입힐program1Dlg* m_MainDlg = (Cini입힐program1Dlg*)theApp.m_pMainWnd;
	g_pServer = this;
...
BOOL CServer::OnInitDialog()
{
	...
	if (m_ListenSocket.Create(26000, SOCK_STREAM)) { // 소켓생성
	...
	{
		//프로그램 스타트
		CString start = ProgramServer2Start(_T("_[Program2]_[Server]"), _T(" "));
		m_Socket.Send((LPVOID)(LPCTSTR)start, start.GetLength() * 2);
		Sleep(3000);
		//서버연결 로그
		CString temp = ConnectServer(_T("_[Program2]_[Server]"), _T(" "));
...
#pragma endregion
	...
	CString temp = Exit(_T("_[Program2]_[Server]"), _T(""));
	m_Socket.Send((LPVOID)(LPCTSTR)temp, temp.GetLength() * 2);
...
void CServer::LoadVerTxt()
{
	...
	BOOL bOpen = m_file.Open(_T("C:\\Temp\\Program2_Version.txt"), CStdioFile::modeCreate | CStdioFile::modeNoTruncate | CStdioFile::modeReadWrite);
	if (!bOpen)
	{
...
void CServer::Readtxt()
{
	...
	if (file.Open(_T("c://temp//LogFolder//Program2log.txt"), CFile::modeRead | CFile::typeText))
	{
		//vec.clear();
		while (file.ReadString(str))
		{
			CString Date, Name, strMsg, asd;
