...
BOOL Cini입힐program1Dlg::OnInitDialog()
{
	...
	GetPrivateProfileString(_T("Program"), _T("Select"), _T("-"), (LPTSTR)cBuf, 256, _T("C:\\Temp\\ProjectSimul.ini"));
	num = atoi(cBuf);
...
void Cini입힐program1Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
	...
	//	if (num == 0)
	//	{
	//		WritePrivateProfileString(_T("Program"), _T("Select"), _T("0"), _T("C:\\Temp\\ProjectSimul.ini"));
	//		CAboutDlg dlgAbout;
	//		dlgAbout.DoModal();
	//	}
	//
	//	else if (num == 1)
	//	{
	//		serverDlg = new CServer;
	//		serverDlg->Create(IDD_SERVER);
	...
	//	else if (num == 2)
	//	{
	//		clientDlg = new CClient;
	//		clientDlg->Create(IDD_CLIENT);
