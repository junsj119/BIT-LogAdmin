// AddGraph.cpp : implementation file
//
#include<iostream>
#include "stdafx.h"
#include "LogAdmin.h"
...
IMPLEMENT_DYNAMIC(CAddGraph, CDialogEx)

CString selectPath = _T("");

CAddGraph::CAddGraph(CWnd* pParent /*=NULL*/)
...
void CAddGraph::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_PointerPB, m_PointerPB);
	DDX_Control(pDX, IDC_UpdatePeriod, m_UpdatePeriod1);
	DDX_Control(pDX, IDC_AlphaValue, m_ValueA1);
	DDX_Control(pDX, IDC_BetaValue, m_ValueB1);
	DDX_Control(pDX, IDC_GammaValue, m_ValueC1);
	DDX_Control(pDX, IDC_STATIC1, m_ChartViewer);
	DDX_Control(pDX, IDC_HScrollBar, m_HScrollBar1);
}


BEGIN_MESSAGE_MAP(CAddGraph, CDialogEx)
	ON_CBN_SELCHANGE(IDC_UpdatePeriod, &CAddGraph::OnCbnSelchangeUpdateperiod)
	ON_WM_CLOSE()
	ON_WM_DESTROY()
	ON_WM_HSCROLL()
	ON_WM_PAINT()
	...
	ON_BN_CLICKED(IDC_PointerPB, &CAddGraph::OnBnClickedPointerpb)
	ON_BN_CLICKED(IDC_PointerPB2, &CAddGraph::OnBnClickedPointerpb2)
END_MESSAGE_MAP()


// CAddGraph message handlers
...
void CAddGraph::OnCbnSelchangeUpdateperiod()
{
	CString s;
...
void CAddGraph::OnClose()
{
	((CLogAdminDlg*)GetParent())->b_LoadGraph = FALSE;
	//DestroyWindow();
	//delete this;
...
void CAddGraph::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
...
HCURSOR CAddGraph::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CAddGraph::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CAddGraph::OnTimer(UINT_PTR nIDEvent)
{
	switch (nIDEvent)
	{
	case DataRateTimer:
...
void CAddGraph::OnBnClickedPointerpb()
{
	m_ChartViewer.setMouseUsage(Chart::MouseUsageScroll);
}


void CAddGraph::OnBnClickedPointerpb2()
{
...
//
void CAddGraph::OnViewPortChanged()
{
...
//
void CAddGraph::OnMouseMovePlotArea()
{
...
void CAddGraph::Readtxt()
{
	...
	{
		KillTimer(1);
		AfxMessageBox("Load Log Please..!");
	}
	else
	{
		if (file.Open(selectPath, CFile::modeRead | CFile::typeText))
		{
			//vec.clear();
...
//
void CAddGraph::OnDataRateTimer()
{
...
//
void CAddGraph::OnChartUpdateTimer()
{
	if (m_currentIndex > 0)
	{
		//
...
//
double CAddGraph::moveScrollBar(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	//
...
//
void CAddGraph::updateControls(CChartViewer *viewer)
{
...
//
void CAddGraph::drawChart(CChartViewer *viewer)
{
...
//
void CAddGraph::trackLineLabel(XYChart *c, int mouseX)
{
...
//
void CAddGraph::loadButtonIcon(int buttonId, int iconId, int width, int height)
{
